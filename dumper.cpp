...
bool dumper::resolve_type_info_definition_table() {
  ...
  // Try WORKING BaseNetworkable pattern (48 8B 0D ? ? ? ? 48 8B 89 ? ? ? ? 48
  // 8B 15 ? ? ? ? 48 8B 49 ? E8 ? ? ? ? 48 85 C0 0F 84 ? ? ? ?)
  __try {
    uint8_t* result = FIND_PATTERN_IMAGE(
        dumper::game_base,
        "\x48\x8B\x0D\xCC\xCC\xCC\xCC\x48\x8B\x89\xCC\xCC\xCC\xCC\x48\x8B\x15"
        "\xCC\xCC\xCC\xCC\x48\x8B\x49\xCC\xE8\xCC\xCC\xCC\xCC\x48\x85\xC0\x0F"
        "\x84\xCC\xCC\xCC\xCC");
    if (is_valid_ptr(result)) {
      ...
      __try {
        ...
        if (is_valid_ptr(resolved)) {
          ...
          dumper::type_info_definition_table =
              (il2cpp::il2cpp_class_t**)resolved;
          uint64_t table_addr = (uint64_t)dumper::type_info_definition_table;
          write_to_log("[PATTERN] Type info table (direct): 0x%llx\n",
                       table_addr);

          if (table_addr > dumper::game_base &&
              table_addr < dumper::game_base + 0x10000000) {
            ...
            if (table_addr > dumper::game_base &&
                table_addr < dumper::game_base + 0x10000000) {
              write_to_log(
                  "[PATTERN] BaseNetworkable pattern successful (double "
                  "deref)!\n");
              return true;
            }
          }
  ...
  // Try the  IL2CPP handle signature (48 8D 0D ? ? ? ? E8 ? ? ? ? 89 45 ? 0F 57
  // C0)
  __try {
    uint8_t* result =
        FIND_PATTERN_IMAGE(dumper::game_base,
                           "\x48\x8D\x0D\xCC\xCC\xCC\xCC\xE8\xCC\xCC\xCC\xCC"
                           "\x89\x45\xCC\x0F\x57\xC0");
    if (is_valid_ptr(result)) {
      ...
      __try {
        ...
        if (is_valid_ptr(resolved)) {
          ...
          if (table_addr > dumper::game_base &&
            ...
            write_to_log(
                "[PATTERN] Pointer out of range, trying as double pointer\n");
            ...
            if (table_addr > dumper::game_base &&
                table_addr < dumper::game_base + 0x10000000) {
              write_to_log(
                  "[PATTERN] temopzso signature successful (double deref)!\n");
              return true;
            }
          }
        }
  ...
  __try {
    uint8_t* result = FIND_PATTERN_IMAGE(
        dumper::game_base,
        "\x48\xF7\xE1\x48\x8B\xCA\x48\xC1\xE9\x04\xBA\x08\x00\x00\x00");
    if (is_valid_ptr(result)) {
      ...
      for (int offset : offsets) {
        __try {
          ...
          if (is_valid_ptr(resolved)) {
            ...
            // Check if the pointer looks valid (should be in GameAssembly
            // range)
            uint64_t table_addr = (uint64_t)dumper::type_info_definition_table;
            if (table_addr > dumper::game_base &&
                table_addr < dumper::game_base + 0x10000000) {
              write_to_log(
                  "[PATTERN] Basic pattern successful with offset %d!\n",
                  offset);
              return true;
            } else {
  ...
  __try {
    ...
    il2cpp::il2cpp_class_t* test_class =
        il2cpp::get_class_by_name("Object", "UnityEngine");
...
void dumper::produce() {
  ...
  if (!resolve_type_info_definition_table()) {
    printf(
        "[Rust Dumper] Failed to resolve type info table, continuing with "
  ...
  DUMPER_SECTION("Functions");
  {
    ...
    if (game_physics_static_class) {
      DUMPER_CLASS_BEGIN_FROM_PTR("GamePhysics_Static",
                                  game_physics_static_class);
      DUMPER_SECTION("Offsets");
      // There are two hit buffers, hitBuffer and hitBufferB. For our use case,
